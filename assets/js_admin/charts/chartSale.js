...
$("#product").on('change', ()=>{
    ...
    if(valor ===""){
        ...
        if(valor ==="1"){ table = "ventahuevo vh";}else{table = "ventacigarro vc";}
        console.log("entre 2 ");
    }
});
...
graficoVertical=(json,periodo)=>{
    ...
    drawChart=()=> {
        ...
        var options = {
          title: 'Total de ventas del año '+json.tiempo+' ( pesos chilenos)',
          is3D: true,
          vAxis: {
            minValue: 0,
...
graficoEntreDias=(periodo1,periodo2)=> { 
    ...
    if(periodo1.fecha && periodo2.fecha){
         if(periodo1.fecha == periodo2.fecha ){
            ...
            let graphic =[['Período', 'Ventas'], array1, array2];// toma el arreglo objeto y lo convierte en un arreglo de arrays 

            drawChart=()=>{
               var data = google.visualization.arrayToDataTable(graphic); // se entre el array con los datos 
    
               var options = {
                      title: 'Gráfico comparativo por períodos(diarios)',
                      is3D:true
                        };
                var chart = new google.visualization.PieChart(document.getElementById('piechart'));
                chart.draw(data, options);  
    ...
    if(periodo1.tiempo1 && periodo2.tiempo1){
        if(periodo1.tiempo1 == periodo2.tiempo1 && periodo1.tiempo2 == periodo2.tiempo2 ){
            ...
            swal({
             ...
             drawChart=()=>{
                  var data = google.visualization.arrayToDataTable(graphic); // se entre el array con los datos 
                  var options = {  title: 'Gráfico comparativo: '+ string1 +" vs "+string2, is3D:true };
                  var chart = new google.visualization.PieChart(document.getElementById('piechart'));
                  chart.draw(data, options);  
             }
...
graficoVerticalDia=(json)=>{
    ...
    drawChart=()=> {
        var data = google.visualization.arrayToDataTable([
          ...
          var options = {
            title: 'Utilidades correspondientes al '+json.fecha+' ( pesos chilenos)',
            is3D: true,
            vAxis: {
              minValue: 0,
