...
getDataForm = () => {
    ...
    $.ajax({
        ...
        success: (xhr) => {
            ...
            let detalle = JSON.parse(xhr[1]['detalle']);
            ...
            eggsHeader.forEach((item, index)=>{
                columns.push({ defaultContent: item.name, "render": function (data, type, row){            
                    if(row.state == 0){
                        return `<input type="number" min="0" value="${row.huevos[index].cantidad}" class="form-control" id="${row.id}_${item.id}" name="${item.id}" placeholder="Ingrese cantidad">`
                    }else{
                        return `<input type="number" min="0" value="${row.huevos[index].cantidad}" class="form-control" id="${row.id}_${item.id}" name="${item.id}" placeholder="Ingrese cantidad" readonly>`
                    }
               }});
            });
            ...
            columns.push({ defaultContent: "total", "render": function (data, type, row){
                if(row.total){
                    let total = totalFormat(row.total);
                    return `<input type="text" value=${total} id="total_${row.id}" disabled class="form-control">`
                }else{
    ...
    if(aux == 0){
        ...
        if(cont != 0){
            ...
            $.ajax({
                data: { data },
                type: "POST",
                url: host_url + "api/routes/updateRoute",
                crossOrigin: false,
                dataType: "json",
                success: (result) => {
                    swal({
...
$("#updateRoute").on("click", updateRoute);
